...
class FrontEndTestCase(TestCase):
    ...
    def setUp(self):
        # current UTC timestamp (Django 5: use python's timezone.utc)
        self.now = datetime.datetime.now(tz=timezone.utc)
        self.timedelta = datetime.timedelta(15)

        author = User.objects.get(pk=1)

        # create 10 posts – first 5 are published
        for i in range(1, 11):
    ...
    def test_list_only_published(self):
        """Home page should list only published posts (first 5)"""
        ...
        for i in range(1, 11):
            ...
            if i < 6:
                # published posts must appear exactly once
                self.assertContains(resp, title, count=1)
            else:
                # unpublished posts must be absent
